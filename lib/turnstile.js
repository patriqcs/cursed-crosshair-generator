...
async function verify(token, remoteIp) {
  if (!isConfigured()) {
    // Fail-CLOSED in Produktion: ohne Captcha keine Submissions annehmen (Spam/Flood-
    // Schutz). Bewusstes Deaktivieren nur per ALLOW_NO_CAPTCHA=true.
    if (process.env.NODE_ENV === 'production' && process.env.ALLOW_NO_CAPTCHA !== 'true') {
