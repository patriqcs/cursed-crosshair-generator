...
function getOrCreateSessionSecret() {
  ...
  storage.writeTextSync(SESSION_SECRET_FILE, generated, { mode: 0o600 });
  return generated;
}
...
function getOrCreateAdminPassword() {
  ...
  storage.writeTextSync(ADMIN_CRED_FILE, fileBody, { mode: 0o600 });

  // Passwort NICHT in die Logs schreiben (Log-Leak). Der Operator liest es einmalig
  // aus der 0600-Datei (Pfad wird beim Boot geloggt) oder setzt ADMIN_PASSWORD.
  // eslint-disable-next-line no-console
  console.log('=== Initial admin password generated — see admin-credentials.txt ===');
...
function buildSessionMiddleware(secret) {
  return session({
    ...
    cookie: {
      httpOnly: true,
      // strict: Admin-Flows brauchen keine Cross-Site-Navigation -> härter gegen CSRF.
      sameSite: 'strict',
      // Standardmäßig Secure (Cloudflare-Tunnel terminiert immer HTTPS, localhost
      // gilt als secure context). Nur für lokales Plain-HTTP per Flag abschaltbar.
